...
df_plot = df_sim_result %>% 
  mutate(FVE_ZIV = mean_h_est,
         ...
         LogTime_ZIM = log(total_time_MCMC),
         LogTime_GCTA = log(total_time_GCTA),
         `FVE coverage_ZIV` = h_est_upper > true_h & h_est_lower < true_h,
         `FVE coverage_ZIM` = up_h_mcmc > true_h & low_h_mcmc < true_h,
         `FVE CI range_ZIV` = h_est_upper-h_est_lower,
         `FVE CI range_ZIM` = up_h_mcmc-low_h_mcmc,
         `PNN coverage_ZIV` = global_pi_upper_1 > true_pi & global_pi_lower_1 < true_pi,
         `PNN coverage_ZIM` = up_global_pi_mcmc > true_pi & low_global_pi_mcmc < true_pi,
         `PNN CI range_ZIV` = global_pi_upper_1-global_pi_lower_1,
         `PNN CI range_ZIM` = up_global_pi_mcmc-low_global_pi_mcmc,
         `Sensitivity: 1.00*pi_ZIM` = sensitivity_100_mcmc,
         `Sensitivity: 1.00*pi_ZIV` = sensitivity_100,
         `Sensitivity: 0.50*pi_ZIM` = sensitivity_50_mcmc,
         `Sensitivity: 0.50*pi_ZIV` = sensitivity_50,
         `Sensitivity: 0.25*pi_ZIM` = sensitivity_25_mcmc,
         `Sensitivity: 0.25*pi_ZIV` = sensitivity_25,
         `FDR: 1.00*pi_ZIM` = sensitivity_100_mcmc,
         `FDR: 1.00*pi_ZIV` = sensitivity_100,
         `FDR: 0.50*pi_ZIM` = FDR_50_mcmc,
         `FDR: 0.50*pi_ZIV` = FDR_50,
         `FDR: 0.25*pi_ZIM` = FDR_25_mcmc,
         `FDR: 0.25*pi_ZIV` = FDR_25,
  ) %>% 
  select(n, `True FVE` = true_h, `True PNN` = true_pi, `outcome` = linear_outcome,
         FVE_GCTA, FVE_ZIV,FVE_ZIM,PNN_ZIV,PNN_ZIM,MAE_ZIV,MAE_LASSO,MAE_RIDGE,`FVE coverage_ZIV`,
         `FVE coverage_ZIM`,`PNN coverage_ZIV` ,`PNN coverage_ZIM`, LogTime_ZIV,LogTime_ZIM,LogTime_GCTA,
         `FVE CI range_ZIV`, `FVE CI range_ZIM`, `PNN CI range_ZIV`,`PNN CI range_ZIM`,
         `Sensitivity: 1.00*pi_ZIM`,`Sensitivity: 1.00*pi_ZIV`,
         `Sensitivity: 0.50*pi_ZIM`,`Sensitivity: 0.50*pi_ZIV`,
         `Sensitivity: 0.25*pi_ZIM`,`Sensitivity: 0.25*pi_ZIV`,
         `FDR: 1.00*pi_ZIM`,`FDR: 1.00*pi_ZIV`,`FDR: 0.50*pi_ZIM`,
         `FDR: 0.50*pi_ZIV`,`FDR: 0.25*pi_ZIM`,`FDR: 0.25*pi_ZIV`,`FVE_GCTA\npositive`
  ) %>% 
  ...
  mutate(n = factor(n),
         outcome = ifelse(outcome == T, "Linear", "Truncated"),
         ...
         Model = factor(Model, levels = c("ZIV", "ZIM", "GCTA", "GCTA\npositive","LASSO", "RIDGE"))
         ) 
...
p_coverage_sim_X = ggplot(data = df_coverage) +
  ...
  scale_color_manual(values = c("ZIV" = colors[1], "ZIM" = colors[2]), name = "Coverage")+
  scale_fill_manual(values = c("ZIV" = colors[1], "ZIM" = colors[2]), name = "CI Range on\noriginal scale") +
...
p_ss_sim_X = df_plot %>% 
  ...
  geom_errorbar(aes(ymin = Est - SD, ymax = Est + SD ),position = position_dodge(width = 0.9), width = .2) +
  #geom_boxplot(outlier.shape = NA)+
  scale_fill_manual(values = c("ZIV" = colors[1], "ZIM" = colors[2]))+
  geom_hline(aes(yintercept = 1,linetype = "100%"))+
  scale_linetype_manual(values = c( "100%" = "dashed"), name = "")+
  scale_y_continuous(limits = c(-0.05,1.1), breaks = c(0,0.5,1), labels = c("0%", "50%", "100%"))+
  ...
  labs(title = "A. Sim X:  feature selection Sensitivity and 1-FDR",y = "Value", x = "Top Percent*pi of features selected")
...
p_ss_real_X = df_ss %>% 
  ...
  labs(title = "B. Real X: feature selection Sensitivity and 1-FDR", x = "Top Percent*pi of features selected")
